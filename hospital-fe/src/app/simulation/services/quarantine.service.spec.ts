...
function testCase(patientsBefore: PatientsRegister,
  ...
  return it(description, () => {
    expect(QuarantineService.mapToSimulation(patientsBefore, patientsAfter, drugs)).toEqual(expected);
  });
}
...
describe('Patients data mapper', () => {
  ...
  testCase({ F: 2 }, { F: 2 }, ['I', 'As'], {
    ...
    patientsStatesDiffs: [{
      state: 'F',
      numberOfPatientsBefore: 2,
      numberOfPatientsAfter: 2
    }]
  ...
  testCase({ F: 2 }, { X: 2 }, ['I', 'As'], {
    ...
    }, { state: 'X', numberOfPatientsBefore: 0, numberOfPatientsAfter: 2 }]
  ...
  it('one state in the beginning, two new states after', () => {
    const res = QuarantineService.mapToSimulation({ X: 6 }, {
      F: 2,
      T: 4
    }, []).patientsStatesDiffs;
  ...
  it('multiple states initially, after some states disappear and some are new', () => {
    const res = QuarantineService.mapToSimulation({ X: 6, T: 3 }, {
      F: 2,
      T: 4
    }, ['I', 'As']).patientsStatesDiffs;
    const expectationOnordered: StateDiff[] = [
      { state: 'F', numberOfPatientsBefore: 0, numberOfPatientsAfter: 2 },
      { state: 'X', numberOfPatientsBefore: 6, numberOfPatientsAfter: 0 },
